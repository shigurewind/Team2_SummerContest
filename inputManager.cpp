...
void InputManager::InitializeDefaultBindings()
{
	...
	// その他---------------------------------------------------------------
	// リセット
	BindAction(ACTION_RESET_POSITION, InputBinding(INPUTDEVICE_KEYBOARD, DIK_R));
	BindAction(ACTION_RESET_POSITION, InputBinding(INPUTDEVICE_DPAD, BUTTON_UP, 0));
...
{
	// 押したどうか
	if (IsActionPressed(ACTION_RESET_POSITION))
	{
		resetHoldTime += deltaTime;
	}
	else
	{
		resetHoldTime = 0.0f;  // 離したらリセット
	}
}



bool InputManager::IsResetTriggered()
{
	// 時間にすぎたらtrue
	if (resetHoldTime >= RESET_HOLD_DURATION)
	{
		resetHoldTime = 0.0f;  // リセット
		return true;
	}
	return false;
}
